...
def get_security_state(request):
    state = CounterMeasure.objects.get(id=1)
    state.hmac = request.GET.get("hmac", None) == "0"
    state.sha385 = request.GET.get("sha385", None) == "0"
...
def perform_extension_attack(
    secret_length, original_data, append_data, original_signature, hash_function="md5"
):
...
def is_valid_md5_signature(signature):
    return len(signature) == 32 and all(
        c in "0123456789abcdef" for c in signature.lower()
    )
...
def home(request):
    ...
    return render(
        request, "hash_extension/home.html", {"images": images_data, state: state}
    )
...
def view_image(request):
    ...
    if verify_signature(secret_key, data, mac, hash_function, use_hmac):
        return render(
            request,
            "hash_extension/view_image.html",
            {"image": image, "is_owner": (download == "true"), state: state},
        )
    else:
        return render(
            request,
...
def perform_attack(request):
    ...
    if request.method == "POST":
        ...
        if not image_id or not original_owner or not original_signature:
            return render(
                request,
                "hash_extension/perform_attack.html",
        ...
        if not verify_signature(
            secret_key, original_data, original_signature, hash_function, use_hmac
        ):
            return render(
                request,
                "hash_extension/perform_attack.html",
        ...
        if not use_hmac:
            ...
            try:
                new_data, new_sig = perform_extension_attack(
                    len(secret_key),
                    original_data,
                    append_data,
                    original_signature,
                ...
                new_valid = verify_signature(
                    secret_key, new_data, new_sig, hash_function, use_hmac
                )
                if new_valid:
                    new_url = (
                        parsed_url.scheme
                        ...
                        + parsed_url.netloc
                        + "/view_image/?"
                        + str(new_data).replace("b'", "").replace("'", "")
                        + "&mac="
                        + new_sig
                    )
                    ...
                    return render(
                        request,
                        "hash_extension/attack_result.html",
                        {"new_url": new_url, "new_sig": new_sig},
                    )
                else:
                    return render(
                        request,
                ...
                return render(
                    request,
                    "hash_extension/perform_attack.html",
                    {"error": f"Extension attack failed: {e}"},
                )
        else:
            return render(
                request,
                "hash_extension/perform_attack.html",
                {"error": "Extension attack not applicable with HMAC"},
            )
